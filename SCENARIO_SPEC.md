...
| 項目 | 状態 |
|---|---|
| `text` の `speed`（文字送り） | ❌ `0`（一括表示）として扱われる |
| ルビ | ✅ 実装済み |
...
保存されるのは `scene` / `command_index` / `variables` / `scenario_version`。
...
| 機能 | 状況 |
|---|---|
| **`chara`（立ち絵）** | 描画 API はあるが、`ScenarioPlayer` に分岐と表示状態の管理が無い |
| **`save` / `load`** | SD 書き込み API は実装済み。状態のシリアライズが未着手 |
