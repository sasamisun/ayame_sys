...
**`scenario.json` に書けて動かないキーは [`choice`](#choice) の `prompt` だけ。**
...
| キー | 型 | 必須 | 内容 |
|---|---|---|---|
| `prompt` | string | | 問いかけ。**現在は表示されない**（下記） |
...
> **`prompt` は現在どこにも表示されない。** 値は保持しているが、
> 描画する場所を本体が持っていない（[8.2](#82-未実装)）。
> 問いかけを見せたい場合は、`choice` の前に [`text`](#text) を置くこと。
...
`body` / `speaker` / `message` と、選択肢の `label` の中で
`{変数名}` が現在の値に置き換わる
（`choice` の `prompt` も置き換えるが、そもそも表示されない）。
...
**動かないキーは [`choice`](#choice) の `prompt` だけ。** ほかはすべて実装済み。

以下はシステム側の未実装項目。

| 機能 | 状況 |
|---|---|
| `choice` の `prompt` の表示 | 値は保持しているが描く場所が無い。当面は前に `text` を置く |
