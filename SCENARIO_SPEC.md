...
| 文書 | 内容 |
|---|---|
| `PROGRAM_SPEC.md` | 現行プログラムの仕様（実装済みのもの） |
| `REFACTORING_LOG.md` | 改修の作業記録 |
| **`SCENARIO_SPEC.md`（本書）** | **これから作るシナリオデータの仕様** |

本書はデータ形式の定義であり、**ローダの実装はまだ存在しない**。
現在シナリオはソースにハードコードされている（`main/hello_world_main.cpp` の `SAMPLE_TEXT`）。
...
凡例: ✅ 既存機能で実現可 / 🔨 ローダ側の新規実装で対応 / ❌ ハードウェア層から新規実装
...
| `speed` | int | `0` | 文字送り速度（ms/字）。`0` で一括表示。**❌ 未実装** |
...
**現在の `SDcard` にファイル書き込み API が存在しない**（`main/SDcard.hpp`）。
公開されているのは読み込み系（`open`/`read`/`seek`/`close`）と
`mkdir`/`remove` のみで、`fopen("wb")` に相当するものが無い。

**セーブ機能の実装には `SDCardWrapper` への書き込み API 追加が前提になる。**
...
本仕様のうち、現在のコードで**実現できるもの**と**新規実装が要るもの**を明示する。
...
| 画像表示 PNG/JPEG/BMP/QOI | `display.drawPngFile(&SD, path, x, y, ...)`。位置・スケール・クリップ指定可 |
| フルリフレッシュ / 残像消去 | `SimpleTransition::refreshScreen()` / `clearGhosting()` |
| 選択肢 UI | `Button` / `ButtonManager` / `TouchHandler` |
| シナリオ一覧の列挙 | `SD.listDir()`（`isDirectory` で判別可） |
| **本文のページ送り** | `TypoWrite::drawTextPaged()`。描き切れなかった位置を返すので、続きを次ページに出せる |
| **ルビ** | `TypoWrite::setRubyEnabled(true)`。半角 `\|漢字<かんじ>` / 全角 `｜漢字《かんじ》` の両対応 |

### 8.2 ローダ側の新規実装で対応（🔨）

| 機能 | 備考 |
|---|---|
| JSON 解析 | **cJSON が ESP-IDF に同梱済み**。`main/CMakeLists.txt` の `REQUIRES` に `"json"` を足すだけで使える。外部ライブラリの追加は不要 |
| 変数・条件式の評価 | `set` / `if` / `cond` |
| シーン遷移の制御 | `jump` / `next` / `end` |
| **セーブの書き込み** | **`SDCardWrapper` に書き込み API の追加が必要**（[6.2](#62-実装上の前提)） |
| ファイル全体の読み込み | `size()` → `open()` → `read()` → `close()` を組む。既存のヘルパは無い |

### 8.3 ハードウェア層から新規実装（❌）

| 機能 | 状況 |
|---|---|
| **ブザー音（`beep`）** | 音声出力の実装が一切無い。M5Unified 未導入のため Speaker API も無い |
| **文字送り（`speed`）** | `TypoWrite::drawText()` は一括描画のみ。1 文字ずつ出す経路が無い |
| **電池残量** | 実装無し |
| **Wi-Fi 配信** | 実装無し。`REQUIRES` にも入っていない |

これらは `format_version` 1 に**記法だけ予約**してある。
実装された時点で既存シナリオを書き直さずに有効になる。
