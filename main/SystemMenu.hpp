 ...
 *
 * ## 一覧では scenario.json を全部は読まない
 *
 * 各シナリオのタイトルを出すために全フォルダの JSON を解析すると、
 * シナリオが増えるほどメニューの表示が待たされる。
 * 一覧は `ScenarioLoader::peekTitle()` で**先頭 4KB だけを読み**、
 * 文字列走査で `meta.title` を拾う。見つからなければフォルダ名を出す。
 * 全体の解析は選ばれてから行う。
 *
 * ## ボタンはこの画面が所有する
 *
