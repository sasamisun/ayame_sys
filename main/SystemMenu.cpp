...
void SystemMenu::shutdown()
{
    ...
    // **この待ちは短くしないこと。**
    //
    // 300ms しか置かずに電源を切っていたときは、
    // 画面上部にランダムな横線が入ることがあった（出ないこともあった）。
    // 待ちを伸ばすと再現しなくなったので、原因は表示の定着待ちが
    // 足りていなかったこと。
    //
    // このパネルは物理 960x540 / 論理 540x960 で物理 x が論理 y に対応するため、
    // 「論理座標の上部の横線」は物理スキャンラインの先頭部分にあたる。
    // 走査が終わりきる前に電源が落ちると、その部分が中途半端に残る。
    //
    // 電源を切るのは頻繁な操作ではないので、確実さを優先して長めに取る。
